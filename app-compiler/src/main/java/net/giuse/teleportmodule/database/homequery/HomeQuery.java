    ...
    public class HomeQuery implements Query {
        ...
        @Override
        public void query() {
            executeQuery.execute(new QueryCallback("CREATE TABLE IF NOT EXISTS lifeserver_home (UUID TEXT,Location TEXT);", PreparedStatement::execute));
            executeQuery.execute(new QueryCallback("SELECT * FROM lifeserver_home", preparedStatement -> {
